...
function coerceAccepts(uarg, desc) {
  ...
  if (conversionNeeded) {
    if (Dynamic.canConvert(targetType)) {
      ...
      try {
        ...
        message = util.format('Invalid value for argument \'%s\' of type ' +
  ...
  var typeMismatch = targetType !== 'any' &&
    ...
    !(Array.isArray(targetType) && actualType === 'array') &&
  ...
  if (argIsNullOrUndefined || uarg === '') {
    if (desc.required) {
      throw badArgumentError('\'' + name + '\' is a required arg.');
    } else {
  ...
  if (actualType === 'number' && Number.isNaN(uarg)) {
    throw badArgumentError('\'' + name + '\' must be a number.');
  }
...
function convertValueToTargetType(argName, value, targetType) {
  switch (typeof value) {
    ...
    case 'object':
      return value;
    default:
      throw badArgumentError(argName + ' must be ' + targetType);
  }
}
